...
var Where = &util.Command{
	...
	Trigger: func(name string, peer_id int, from_id int) (err error) {
		...
		if status.Session.Online {
			...
			message := fmt.Sprintf("Игрок %s был в сети %s (Сессия длилась %d минут)", mojang.Name, time.Unix(status2.Player.LastLogout/1000, 0).Format("02.01.2006, в 15:04"), ((status2.Player.LastLogout-status2.Player.LastLogin)/1000)/60)
			err = util.SendMessage(peer_id, message)
		}
		return
	},
